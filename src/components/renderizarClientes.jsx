    ...
    : repositoryClientes;
        ...
        return clientesFiltrados.map((cliente) => (
            <div key={cliente.codigo} className="flex items-center space-evenly bg-gray-200 rounded-md p-4 mb-4 w-4/5 *:ms-4 cursor-pointer" onClick={()=>navigate(`/cliente/${cliente.codigo}`)}>
                <p className="text-blue-500 font-bold">{cliente.codigo}.</p>
                <p className="w-72"><strong>Nome: </strong>{cliente.nome}</p>
                {cliente.nome_fantasia !== null && (<p><strong>Fantasia: </strong>{cliente.nome_fantasia}</p>)}
                <p><strong>Doc: </strong>{cliente.cnpj_cpf}</p>
